        ...
        if not self.state or self.state == 'draft':
            ...
            if order:
                ...
                try:
                    ...
                    dic['move_id'] = invoice.id if invoice else False
...
class sv_route_dispatch_line(models.Model):
    ...
    move_id = fields.Many2one(comodel_name='account.move',string="Factura",copy=False)
    invoice_date = fields.Date("Fecha factura",related='move_id.invoice_date')
    invoice_number = fields.Char("Número de factura",related='move_id.doc_numero')
